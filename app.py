...
class ImageProcessor:
        ...
        def do_right(self):
                self.image = self.filtler(Image.ROTATE_90)
                self.saveImage()
                image_path = os.path.join(self.dir,self.save_dir, self.filename)
                self.showImage(image_path)

        def add_texture(selfself):
            texture = Image.open("img_anetoklaznyl.png")
            width, height = self.Image.size
            texture = texture.resieze((width, height))
            self.Image.paste.paste(texture, (0, 0), texture)
